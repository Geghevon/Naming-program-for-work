...
ft_putstr_fd_while(char *str, int fd)
{
	int	i;
...
void ft_writing(t_helping help)
{
	...
	i = help.game_cont - 1;
	fd = open(ft_strjoin(ft_substr(help.raw_time_day, 0 , 10), " FIFA.txt"), O_CREAT | O_WRONLY | O_TRUNC);
	if (fd < 1)
	{
		list_free(&help);
		exit(EXIT_FAILURE);
	...
	while(i >= 0)
	{
		...
		ft_putstr_fd_while(help.raw_player_team_1[i], fd);
		ft_putstr_fd(help.utils[1], fd);
		ft_putstr_fd_while(help.raw_player_team_2[i], fd);
...
int	main()
{
	...
	while (help.raw_time_day[i])
	{
		ft_putstr_fd(help.raw_time_day[i],1);
		ft_putchar_fd('\n',1);
		i++;
	}
